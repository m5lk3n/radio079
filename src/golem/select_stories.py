...
from config import GOLEM_STORIES_JSON, GOLEM_PODCAST_STORIES_JSON, OPEN_API_MODEL
...
def select_golem_stories():
    client = OpenAI(
        api_key=os.environ["OPENAI_API_KEY"],
        base_url=os.environ.get("OPENAI_BASE_URL"),
    )

    with open(GOLEM_STORIES_JSON, "r", encoding="utf-8") as f:
        stories = json.load(f)
    ...
    with open(GOLEM_PODCAST_STORIES_JSON, "w", encoding="utf-8") as f:
    ...
    print(f"\nWrote {GOLEM_PODCAST_STORIES_JSON}")
